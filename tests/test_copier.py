...
def test_template_generation_scratch(tmp_path: Path):
    ...
    assert 'name = "clear-skies-akeyless-custom-test-service"' in pyproject_toml_content
    ...
    expected_package_dir = (
        project_destination / "src" / "clearskies_akeyless_custom_test-service"
    )
    assert not expected_package_dir.is_dir()

    # Check the generated directory or import path:
    expected_package_dir = (
        project_destination / "src" / "clearskies_akeyless_custom_test_service"
    )
    ...
    expected_files = [
        "README.md",
        "src/clearskies_akeyless_custom_test_service/__init__.py",
    ]
